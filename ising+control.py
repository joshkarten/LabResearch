...
K = 1
# update this according to the number of numbers generated
chains = 2
    ...
    for prob in range(low_prob, high_prob, 1):
        ...
        for i in range((length**2)//2):
            ...
            if random.random() > (float(prob)/100):
                ...
                '''
            # initializing ising lattice. this is most likely a non-optimal step due to the conversions
            ...
            for x_pos in range(0, chains):
                for y_pos in range(0, length):
                    E_i = energy(x_pos, y_pos, lattice, chains, length, 1, True)
                    old = lattice[x_pos]
                    # noinspection PyTypeChecker
                    lattice[x_pos] = format(int(lattice[x_pos], base=2) ^ (1 << (length - y_pos - 1)), '0' + str(length) + 'b')
                    E_f = energy(x_pos, y_pos, lattice, chains, length, K, True)
                    if random.random() <= boltzmann_probability(E_i, E_f, Beta):
                        pass
                    else:
                        lattice[x_pos] = old
            # likely can find a way to avoid converting back and forth
...
#print(record[99])
fig, ax = plt.subplots()
ax.plot( np.linspace(0,1,high_prob-low_prob), (record1+record2)/2, marker='o', label="Total Param")
...
#plt.show()
fig, ax = plt.subplots()
ax.plot( np.linspace(.3,.6,30), record1[30:60]/2+record2[30:60]/2, marker='o')
ax.set_xlabel(r'Probability of Selecting the Control Map')
ax.set_ylabel(r'$ <\hat{O}>$')
ax.set_title(r'Results of a Simulation with Bitstring length '+str(length))
#fig.save("200 Length BitString, Full Probability Spectrum")
